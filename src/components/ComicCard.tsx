...
export default function ComicCard({ comic, onBuy, hasDigitalAccess = false, onRead }: ComicCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[3/4] w-full bg-neutral-950 overflow-hidden border-b border-white/10 flex items-center justify-center">
        ...
        <div className="pt-4 border-t border-white/5 flex items-center justify-between gap-4">
          ...
          {hasDigitalAccess && onRead ? (
            <button
              ...
              className="px-4 py-2.5 rounded bg-gradient-to-r from-ocu-gold to-yellow-500 hover:brightness-110 text-black font-display text-xs font-bold tracking-widest uppercase flex items-center gap-2 cursor-pointer shadow-md shadow-ocu-gold/10 transition-all active:scale-95"
            >
            ...
            <button
              ...
              className="group px-4 py-2.5 rounded bg-white/5 hover:bg-white text-white hover:text-black border border-white/10 hover:border-white transition-all duration-300 font-display text-xs font-bold tracking-widest uppercase flex items-center gap-2 cursor-pointer active:scale-95"
            >
              <ShoppingCart size={13} className="text-ocu-crimson group-hover:text-black transition-colors" />
              <span>{isReleased ? 'Buy Now' : 'Pre-Order'}</span>
            </button>
          )}
        </div>
      </div>
